...
std::map<vec, node> node_graph;

void add_node(vec p) {
	node& main = node_graph[p];
	if (main.p == vec{0, 0}) {
		main = node{p};
	}

	if (main.neighbours.size() > 0) return;

	// Check each of the four directions if theres a connection
	for (int dir = 0; dir < 4; dir++) {
		...
		while (true) {
			...
			if (map_get(l_side) == '.' || map_get(r_side) == '.') {
				// Handle intersection
				main.neighbours.push_back(&node_graph[walk]);
				add_node(walk);
				break;
			}
		}
...
// std::vector<std::vector<node*>> all_paths(vec from, vec to) {
// 	std::vector<std::vector<node*>> paths;
// 	std::vector<node*> path;
// 
// 	path.push_back(&node_graph[from]);
// 	// Iterate all child nodes (without looping) depth first
// 	while (true) {
// 
// 	}
// }

void add_loc(vec p, int dir) {
