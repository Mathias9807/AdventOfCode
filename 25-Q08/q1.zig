...
pub fn main() void {
    ...
    // Iterate pairs
    var pairsList: std.DoublyLinkedList = .{};
    const P = struct { data: [3]i64, node: std.DoublyLinkedList.Node };
    var firstPair = P{
        .data = .{coords.items[0].dist(coords.items[1]), 0, 1},
        .node = .{},
    };
    pairsList.prepend(&firstPair.node);
    ...
    const iterMaxBound = @as(u32, @intFromFloat(1000 * 5));
    ...
    for (coords.items, 0..) |a, i| {
        ...
        for (coords.items[i+1..], i+1..) |b, j| {
            ...
            const dist = a.dist(b);
            var it = pairsList.first;
            var k: u32 = 0;
            while (it) |node| : (it = node.next) {
                const pair: *P = @fieldParentPtr("node", node);
                const p = pair.data;
                if (dist < p[0]) {
                    const newPair = alloc.create(P) catch unreachable;
                    newPair.* = P{.data = .{dist, I64(i), I64(j)}, .node = .{}};
                    pairsList.insertBefore(node, &newPair.node);
                    break;
                }
                k += 1;
    ...
    var pairs = std.ArrayList([3]i64).initCapacity(alloc, 1000) catch unreachable;
    var it = pairsList.first;
    while (it) |node| : (it = node.next) {
        const pair: *P = @fieldParentPtr("node", node);
        pairs.append(alloc, pair.data) catch unreachable;
    }
