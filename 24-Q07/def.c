...
ListEntry* ListInsert(List* l, void* value, int index) {
	ListEntry* new = calloc(1, sizeof(ListEntry));
	new->value = value;

	if (index == 0) {
		ListEntry* next = l->first;
		...
		ListEntry* prev = l->first;
		int prevIndex = 0;
		while (++prevIndex != index && prev->next) prev = prev->next;
